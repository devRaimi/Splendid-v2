...
export default function OrderPage() {
	...
	useEffect(() => {
		if (data) {
			data.forEach((item) => {
				item.available ? setProducts((prev) => [...prev, item]) : null;
			});
		}
	}, [data]);
	...
	const cart = useMemo(
		() =>
			products
				?.filter((p) => (qty[p._id] ?? 0) > 0)
				.map((p) => ({
					...p,
					qty: qty[p._id],
					subtotal: qty[p._id] * p.size.price,
				})),
		[qty],
	);
	...
	const place = async () => {
		if (isGift) {
			if (!form.senderPhone.trim())
				return toast('Enter your WhatsApp number', true);
			if (!form.recipientPhone.trim())
				return toast("Enter the recipient's WhatsApp number", true);
			if (!form.recipientName.trim())
				return toast("Enter recipient's name", true);
			if (!form.recipientLocation.trim())
				return toast('Enter delivery location', true);
		} else {
			if (!form.name.trim()) return toast('Enter your name', true);
			if (!form.phone.trim())
				return toast('Enter your WhatsApp number', true);
			if (!form.location.trim())
				return toast('Enter pickup location', true);
		}
		...
		formData.append('itemsOrdered', JSON.stringify(items));
		try {
	...
	return (
		<div className=' bg-paper flex flex-col'>
			...
			<header className='sticky top-0 z-40 bg-paper/95 backdrop-blur-sm border-b border-paper-border'>
				<div className='w-full sm:w-[75%] md:w-1/2 mx-auto px-4 py-3 flex items-center justify-between'>
					<div className='flex items-center gap-3'>
						<div className='w-8 h-8 bg-puff rounded-lg flex items-center justify-center text-white text-sm font-display font-bold'>
							SP
						</div>
							...
							<button
								...
								className={`px-4 py-1.5 rounded-lg text-[13px] font-semibold capitalize transition-all ${
									tab === t
										...
										: 'text-paper-muted hover:text-ink-soft'
								}`}
							>
								{t}
							</button>
			...
			<main className='w-full sm:w-[75%] md:w-1/2 mx-auto flex-1 px-4 pb-35'>
				{tab === 'track' ? (
					<TrackPanel />
				) : (
					<>
...
const Step1 = ({
	...
	return (
		<div className='pt-6 space-y-6 h-max'>
			<div>
					...
					{[
						...
						<button
							...
							className={`p-4 rounded-2xl border-2 text-left transition-all ${
								orderType === t
									? 'border-puff bg-puff/5'
									: 'border-paper-border bg-white hover:border-paper-muted'
							}`}
						>
							...
							<p className='text-[12px] text-paper-muted mt-0.5'>
								{sub}
							</p>
						</button>
					))}
				...
				{isGift && (
					<div className='mt-3 flex gap-3 p-4 rounded-xl bg-amber-50 border border-amber-100 animate-pop'>
						...
						<p className='text-[13px] text-amber-800 leading-relaxed'>
							<strong>Your name stays hidden.</strong> We only
							need your WhatsApp to confirm payment — the
							recipient won't know it's from you.
						</p>
					</div>
				)}
			</div>
			<div>
				<Label>
					Location <span className='text-red-400 text-base'>*</span>
				</Label>
				...
				<div>
					...
					<div className='grid sm:grid-cols-2 grid-cols-1 gap-3 mt-2 h-max'>
						{products
							.filter(
								(p) =>
									p.locationAvailable === campus ||
									p.locationAvailable === 'both',
							)
							.map((p) => (
								<ProductTile
									key={p._id}
			...
			{/* {needsFlavour && (
				<div className='animate-pop'>
					<Label>
						Flavour <span className='text-red-400'>*</span>
					</Label>
						...
						<div className='flex items-center justify-between mb-3 px-1'>
							<span className='text-[13px] text-paper-muted font-medium'>
								{cart
									.map(
										(i) =>
											`${i.name}(${i.size.sizeName}) ×${i.qty}`,
									)
									.join('  ')}
							</span>
							<span className='font-display font-bold text-ink text-[16px]'>
								{formatNaira(total)}
							</span>
						</div>
					)}
...
const Step2 = ({
	...
	return (
		<div className='pt-5 space-y-5 pb-8'>
			<button
				onClick={() => setStep(1)}
				className='flex items-center cursor-pointer gap-1.5 text-base duration-500 text-paper-muted hover:text-ink font-medium transition-colors'
			>
				← Back to items
			</button>
			...
			{isGift ? (
				<>
					<Section
						title='🕵️ Your contact'
						subtitle='Only used to confirm payment — never shared with recipient'
					>
					...
					<Section
						...
						subtitle='Who is receiving this?'
					>
						...
						<Field label='WhatsApp number'>
							<Input
								type='tel'
								placeholder='080... (for delivery notification)'
						...
						<Field label='Delivery location'>
							<Input
								...
								onChange={set('recipientLocation')}
							/>
						</Field>
						<Field label='A secret note'>
							<Textarea
			...
			<div>
				...
				<div className='mt-2 bg-white border-2 border-dashed border-paper-border rounded-2xl overflow-hidden'>
					...
					{cart.map((i, idx) => (
						<div
							key={i.id}
							className={`flex items-center justify-between px-4 py-3 ${idx < cart.length - 1 ? 'border-b border-paper-border/50' : ''}`}
						>
							<div>
								<span className='text-[14px] font-medium text-ink'>
									{`${i.name} (${i.size.sizeName})`}
								</span>
								<span className='ml-2 font-mono text-[12px] text-paper-muted'>
									×{i.qty}
								</span>
							</div>
					...
					<div className='flex items-center justify-between px-4 py-3'>
						<span className='font-display font-bold text-[13px] uppercase tracking-wider text-ink'>
							Total
						</span>
						<span className='font-mono font-bold text-[20px] text-puff'>
							{formatNaira(total)}
						</span>
					</div>
				</div>
				...
				<div className='mt-2 bg-night rounded-2xl p-5 text-white'>
					...
					<div className='mt-4 rounded-xl bg-white/8 border border-white/10 px-4 py-3'>
						<p className='text-[12.5px] text-white/70 leading-relaxed'>
							...
							as the transfer narration so we can match your
							payment.
						</p>
					</div>
				</div>
			...
			<div>
				...
				<label
					className={`mt-2 flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed py-8 px-4 text-center cursor-pointer transition-all ${
						receipt
							? 'border-green-400 bg-green-50'
							: 'border-paper-border bg-white hover:border-puff/40 hover:bg-puff/[0.03]'
					}`}
				>
			...
			<div className='fixed bottom-0 inset-x-0 p-4 bg-paper/95 backdrop-blur-sm border-t border-paper-border z-30'>
				<div className='w-full sm:w-[75%] md:w-1/2 mx-auto'>
	...
	return (
		<div className='animate-pop pt-16 pb-10 text-center'>
			...
			<h1 className='font-display font-bold text-[28px] text-ink'>
				{isGift ? 'Gift queued!' : 'Order placed!'}
			</h1>
			...
			<div className='mt-8 mx-auto max-w-[240px] bg-white border-2 border-dashed border-paper-border rounded-2xl overflow-hidden'>
				<div className='px-5 pt-5 pb-3 border-b border-dashed border-paper-border'>
					<p className='font-mono text-[10px] text-paper-muted uppercase tracking-widest mb-1'>
						Reference
					</p>
					<p className='font-mono font-bold text-[32px] text-ink tracking-wider leading-none'>
						{doneRef}
					</p>
				</div>
			...
			<p className='mt-3 text-[12px] text-paper-muted'>
				Screenshot this to track your order
			</p>
			...
			<div className='mt-8 space-y-3 max-w-xs mx-auto'>
				<a
					...
					rel='noreferrer'
				>
					<CTA className='!bg-[#25D366] hover:!bg-[#1da851]'>
...
const TRACK_STEPS = ['pending', 'confirmed', 'approved', 'completed'];
const TRACK_META = {
	pending: {
		icon: '📥',
		label: 'Received',
	...
	confirmed: {
		icon: '✅',
		label: 'Confirmed',
		sub: 'Order confirmed, Verifying payment...',
	},
	approved: {
		icon: '🍩',
		label: 'approved',
		sub: 'Payment Confirmed, preparing package...',
	},
	completed: { icon: '🎉', label: 'Done', sub: 'Hope you enjoyed it! 🧡' },
};

function TrackPanel() {
	const [fetchData] = useLazyFetchOrderByRefQuery();
	const toast = useToast();
	...
	useEffect(() => {
		result
			? setStatus(
					result?.status.isConfirmed
						? 'confirmed'
						: result?.status.isApproved
							? 'approved'
	...
	async function lookUp() {
		...
		try {
			setLoading(true);
			await setResult(
				(await fetchData(ref.trim()).unwrap()) ?? 'notfound',
			);
		} catch {
	...
	const idx =
		result && result !== 'notfound' ? TRACK_STEPS.indexOf(status) : -1;
			...
			{result === 'notfound' && (
				<div className='animate-pop rounded-2xl border border-paper-border bg-white p-6 text-center'>
					...
					<p className='text-[13px] text-paper-muted mt-1'>
						Check the reference number from your confirmation
						screen.
					</p>
				</div>
			)}
			...
			{result && result !== 'notfound' && (
				<div className='animate-pop bg-white border-2 border-dashed border-paper-border rounded-2xl overflow-hidden'>
					<div className='px-5 py-4 border-b border-dashed border-paper-border flex items-start justify-between'>
						<div>
							<p className='font-mono font-bold text-[22px] text-ink'>
								{result.ref}
							</p>
							...
							return (
								<div key={s} className='flex gap-4'>
									<div className='flex flex-col items-center'>
										<div
											className={`w-8 h-8 rounded-full flex items-center justify-center text-[13px] shrink-0 transition-all ${done ? 'bg-puff text-white' : 'bg-paper-warm text-paper-muted'}`}
										>
											{done
												? active
													? meta.icon
										...
										{i < TRACK_STEPS.length - 1 && (
											<div
												className={`w-0.5 flex-1 min-h-[20px] my-1 ${i < idx ? 'bg-puff/40' : 'bg-paper-border'}`}
											/>
										)}
									</div>
									<div className='pt-1 pb-5'>
										<p
											className={`text-[14px] font-semibold ${done ? 'text-ink' : 'text-paper-muted'}`}
										>
											{meta.label}
										</p>
										{active && (
...
function Label({ children }) {
	return (
		<p className='text-[11px] font-bold uppercase tracking-widest text-paper-muted'>
			{children}
		</p>
	);
}
...
function Section({ title, subtitle, children }) {
	return (
		<div className='rounded-2xl border border-paper-border bg-white p-5'>
			...
			{subtitle && (
				<p className='text-[12.5px] text-paper-muted mt-0.5 mb-4'>
					{subtitle}
				</p>
			)}
...
const inputBase =
	'w-full rounded-xl border border-paper-border bg-paper px-4 py-3 text-[14px] text-ink placeholder:text-paper-muted/50 outline-none focus:border-puff focus:ring-2 focus:ring-puff/15 transition';
...
function Pill({ active, onClick, children }) {
	return (
		<button
			type='button'
			onClick={onClick}
			className={`px-4 py-2 capitalize rounded-full cursor-pointer duration-500 border text-base font-medium transition-all active:scale-95 ${
				active
...
function CTA({ children, onClick, disabled, className = '' }) {
	return (
		<button
			...
			onClick={onClick}
			disabled={disabled}
			className={`w-full bg-puff hover:bg-puff-dim text-white font-display font-bold text-base p-4 duration-500 cursor-pointer rounded-2xl transition-all active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none shadow-lg shadow-puff/20 ${className}`}
		>
			{children}
		</button>
	);
...
function ProductTile({ product: p, count, onBump }) {
	return (
		<div
			className={`relative h-max bg-white rounded-lg border cursor-pointer duration-500 border-gray-200 transition-all ${count > 0 ? 'border-puff' : ' hover:shadow-md'}`}
		>
			{count > 0 && (
			...
			<div
				style={{ background: `url(${p?.image.url})` }}
				className={`bg-cover! bg-center! w-full h-40 rounded-t-lg bg-paper flex items-center justify-center`}
			>
				{/* {p.image.url} */}
			</div>
			<div className='p-4'>
				<p className='font-semibold flex flex-col mb-2 items-center text-base text-ink'>
					{p.name}{' '}
				...
				<p className='font-mono font-bold text-[14px] text-puff mt-0.5'>
					{formatNaira(p.size.price)}
				</p>
				{count > 0 ? (
					<div className='flex items-center justify-between mt-3 bg-paper rounded-xl px-1 py-0.5'>
						<button
							onClick={() => onBump(p._id, -1)}
							className='w-7 h-7 rounded-lg bg-white text-ink font-bold text-[16px] flex items-center justify-center hover:bg-puff hover:text-white transition'
						>
							−
						</button>
						...
						<button
							onClick={() => onBump(p._id, 1)}
							className='w-7 h-7 rounded-lg bg-puff text-white font-bold text-[16px] flex items-center justify-center hover:bg-puff-dim transition'
						>
							+
						</button>
					</div>
					...
					<button
						onClick={() => onBump(p._id, 1)}
						className='w-full mt-3 py-1.5 rounded-xl border border-paper-border text-[13px] font-semibold text-paper-muted hover:border-puff hover:text-puff transition'
					>
						Add
					</button>
				)}
...
function StatusBadge({ status }) {
	...
	return (
		<span
			className={`px-2.5 py-1 rounded-full text-[11px] font-bold capitalize ${map[status] ?? 'bg-zinc-100 text-zinc-500'}`}
		>
			{status}
		</span>
	);
